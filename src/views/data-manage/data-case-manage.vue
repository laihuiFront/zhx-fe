  ...
  >
    <el-form ref="form" :model="formInline" :inline="true" class="query-wrap queryStyle">
      ...
      <el-form-item v-if="queryConf.wtf || queryConfFlag" label="委托方：">
        <el-select v-model="formInline.clients" filterable collapse-tags multiple placeholder="请选择委托方" clearable @change="clientCurrent">
          <el-option
      ...
      <el-form-item v-if="queryConf.cjh || queryConfFlag" label="车架号：">
        <el-input v-model="formInline.vin" placeholder="请输入车架号" />
      </el-form-item>
      <el-form-item v-if="queryConf.pzh || queryConfFlag" label="牌照号：">
        <el-input v-model="formInline.license" placeholder="请输入牌照号" />
      </el-form-item>
      <el-form-item v-if="queryConf.cssb || queryConfFlag" label="催收手别：">
        <el-input v-model="formInline.collectHand" placeholder="请输入催收手别" />
      </el-form-item>
      ...
      <el-form-item v-if="queryConf.sqdh || queryConfFlag" label="申请单号：">
        <el-input
          v-model="formInline.applyOrderNo"
          type="textarea"
          placeholder="请输入申请单号"
          style="width: 100%;"
          rows="3"
      ...
      <el-form-item v-if="queryConf.khh || queryConfFlag" label="开户行：">
        <el-input v-model="formInline.bank" placeholder="请输入开户行" />
      </el-form-item>
      ...
      <el-form-item v-if="queryConf.waje || queryConfFlag" label="委案金额下限：">
        <el-input v-model="formInline.moneyStart" type="number" placeholder="请输入委案金额下限" />
      </el-form-item>
      <el-form-item v-if="queryConf.waje || queryConfFlag" label="委案金额上限：">
        <el-input v-model="formInline.moneyEnd" type="number" placeholder="请输入委案金额上限" />
      </el-form-item>
      <el-form-item v-if="queryConf.yqts || queryConfFlag" label="逾期天数下限：">
        <el-input v-model="formInline.overdueDaysStart" type="number" placeholder="请输入逾期天数下限" />
      </el-form-item>
      <el-form-item v-if="queryConf.yqts || queryConfFlag" label="逾期天数上限：">
        <el-input v-model="formInline.overdueDaysEnd" type="number" placeholder="请输入逾期天数上限" />
      </el-form-item>
      ...
      <el-form-item v-if="queryConf.csjl || queryConfFlag" label="催收记录：">
        <el-input v-model="formInline.collectInfo" placeholder="请输入催收记录" />
      </el-form-item>
      <el-form-item v-if="queryConf.bm || queryConfFlag" label="部门：">
        <el-autocomplete v-model="deptName" width="200" clearable placeholder="请输入部门" :fetch-suggestions="showDeptName" @select="selectDept">
          <el-button slot="append" icon="el-icon-search" @click="onClickSelectDept" />
        </el-autocomplete>
      </el-form-item>
      <el-form-item v-if="queryConf.csy || queryConfFlag" label="催收员：">
        <el-autocomplete v-model="formInline.odvNameFiter" width="160" clearable placeholder="请输入催收员" :fetch-suggestions="showOdvName" @select="selectOdv">
          <el-button slot="append" icon="el-icon-search" @click="onClickSelectUser3" />
        </el-autocomplete>
      </el-form-item>
      ...
      <el-form-item v-if="queryConf.dq || queryConfFlag" label="地区：">
        <el-select v-model="formInline.province" placeholder="请选择地区" clearable @change="showCity()">
          <el-option
      ...
      <el-form-item v-if="queryConf.dq || queryConfFlag" label="市：">
        <el-select v-model="formInline.city" placeholder="请选择市" clearable @change="showcounty()">
          <el-option
      ...
      <el-row>
        ...
        <el-form-item>
          <el-dropdown v-dropdown-patch v-has="'分案'" placement="bottom" @command="fenancheck">
            <el-button type="primary">
              分案<i class="el-icon-arrow-down el-icon--right" />
            </el-button>
        ...
        <el-form-item>
          <el-dropdown v-dropdown-patch v-has="'案件'" @command="guanlianjian">
            <el-button type="primary">
              案件<i class="el-icon-arrow-down el-icon--right" />
            </el-button>
            <el-dropdown-menu slot="dropdown">
        ...
        <el-form-item>
          <el-dropdown v-dropdown-patch v-has="'修改'" placement="bottom" @command="handleCommand">
            <el-button type="primary">
        ...
        <el-form-item>
          <el-button v-has="'添加评语'" type="primary" @click="addshow">添加评语</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-has="'申请协催'" type="primary" @click="xiecui">申请协催</el-button>
        </el-form-item>
        <el-form-item>
          <el-dropdown v-dropdown-patch v-has="'导出'" placement="bottom" @command="handleExport">
            <el-button type="primary">
        ...
        <el-form-item>
          <el-dropdown v-dropdown-patch v-has="'案件标色'" @command="biaose">
            <el-button type="primary">
              案件标色<i class="el-icon-arrow-down el-icon--right" />
            </el-button>
            <el-dropdown-menu slot="dropdown">
    ...
    <el-dialog
      ...
      width="90%"
    >
      <case-detail :id="detailId" ref="detail" />
    </el-dialog>
    <el-dialog
      v-dialogDrag
      title="添加评语"
      ...
      width="40%"
    >
      <el-form ref="formInline" :inline="true" :model="formInline" class="demo-form-inline" label-width="120px">
        <div class="grid-content bg-purple">
          <el-form-item label="添加评语" :rules="{required: true, message: '评语不能为空', trigger: 'blur'}">
            <el-input v-model="value12" type="textarea" style="width: 200%;" placeholder="请输入评语" />
          </el-form-item>
        </div>
      </el-form>
    ...
    <el-dialog
      ...
      width="30%"
    >
      <el-form ref="fenan" :inline="true" :model="fenan" class="demo-form-inline" label-width="120px">
        <div class="grid-content bg-purple">
          <el-form-item label="催收员" prop="odv">
            <el-input v-model="odvName" width="200" clearable placeholder="请选择催收员" @focus="onClickSelectUser" />
          </el-form-item>
        </div>
      </el-form>
    ...
    >
      <el-form ref="fenan" :inline="true" :model="fenan" class="demo-form-inline" label-width="120px">
        ...
        <div v-if="showSendVisible1" class="filter">
          <h1>分配选项</h1>
          <el-checkbox-group v-model="fenan.sendType">
            <el-checkbox v-for="item in Aoptions" :key="item.id" :label="item.id">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </div>
        <div v-if="showSendVisible1" class="filter">
          <h1>分配方式</h1>
          <el-radio-group v-model="fenan.mathType">
            <el-radio v-for="item in Boptions" :key="item.id" :label="item.id">{{ item.name }}</el-radio>
          </el-radio-group>
        </div>
        <div v-if="showSendVisible1" class="filter">
          ...
          <ul class="salesman-ul">
            <li v-for="(item,index) in fenan.odvNames" :key="item.id">
              <p>{{ item }}（业务员）<el-input v-model="fenan.odvPercent[index]" type="number" />%</p>
            </li>
          </ul>
        </div>
        <div v-if="showSendVisible2" class="filter">
      ...
      <span slot="footer" class="footer">
        <el-button v-if="showSendVisible1" type="primary" @click="showAutoSendByProperty('fenan')">下一步</el-button>
        <el-button v-if="showSendVisible2" type="primary" @click="autoSendByProperty('fenan')">确认分配</el-button>
        <el-button v-if="showSendVisible2" type="primary" @click="last('fenan')">上一步</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      ...
      width="30%"
    >
      <el-form ref="formInline1" :inline="true" :model="formInline1" class="demo-form-inline" label-width="120px">
        <div class="grid-content bg-purple">
          <el-form-item
            label="案件等级"
            prop="importLeave"
            :rules="{required: true, message: '请选择案件等级', trigger: 'blur'}"
          >
    ...
    >
      <el-form ref="formInline1" :inline="true" :model="formInline1" class="demo-form-inline" label-width="120px">
        <div class="grid-content bg-purple">
          ...
          >
            <el-input v-model="formInline1.mVal" style="width: 100%;" placeholder="请输入提成值" rows="4" />
          </el-form-item>
        </div>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    >
      ...
      <el-row ref="boxWrapper" class="pad">
    ...
    <el-dialog
      ...
      class="dialog-wrap"
    >
      <div style="margin-bottom: 10px;"><span style="cursor: pointer;" @click="selectAllTelExport">全选</span><span style="cursor: pointer;margin-left:10px;" @click="selectUnAllTelExport">反选</span></div>
      <el-row ref="boxWrapper" class="pad">
    ...
    <el-dialog
      ...
      class="dialog-wrap"
    >
      <div style="margin-bottom: 10px;"><span style="cursor: pointer;" @click="selectAllCollectExport">全选</span><span style="cursor: pointer;margin-left:10px;" @click="selectUnAllCollectExport">反选</span></div>
      <el-row ref="boxWrapper" class="pad">
    ...
    <el-dialog
      ...
      class="dialog-wrap"
    >
      <div style="margin-bottom: 10px;"><span style="cursor: pointer;" @click="selectAllExport">全选</span><span style="cursor: pointer;margin-left:10px;" @click="selectUnAllExport">反选</span></div>
      <el-row ref="boxWrapper" class="pad">
...
export default {
  ...
  methods: {
    ...
    saveConf() {
      ...
      const queryObj = { module: 'data-case-manage', menu: this.queryConf }
    ...
    saveExportCaseConf() {
      const queryObj = { module: 'data-case-manage-exportCase', menu: this.exportConf }
      saveSelectFilter(queryObj).then(data => {
      })
    },
    ...
    saveExportTelConf() {
      const queryObj = { module: 'data-case-manage-exportTel', menu: this.exportTelConf }
      saveSelectFilter(queryObj).then(data => {
      })
    },
    ...
    saveExportCollectConf() {
      const queryObj = { module: 'data-case-manage-exportCollect', menu: this.exportCollectConf }
      saveSelectFilter(queryObj).then(data => {
      })
    },
    ...
    onClickSaveUser() {
      const selectDataArr = this.$refs.tree.getCurrentNode()
    ...
    onClickSaveUser2() {
      const selectDataArr = this.$refs.tree.getCheckedNodes()
      let selectUserNames = []
      let selectUserIds = []
      let selectPercent = []
    ...
    guanlianjian(command) {
      if (this.deleteList.length >= 1) {
        ...
        for (var i = 0; i < this.deleteList.length; i++) {
          const dataObject = { id: '', status: command }
    ...
    fenancheckone() {
      ...
      const datasList = []
      for (var i = 0; i < this.deleteList.length; i++) {
        const dataObject = { id: '', odv: this.fenan.odv }
        dataObject.id = this.deleteList[i].id
        datasList.push(dataObject)
      }
    ...
    sureAddShow7() {
      const datasList = []
      for (var i = 0; i < this.deleteList.length; i++) {
        const dataObject = { id: '', mVal: this.formInline1.mVal }
        dataObject.id = this.deleteList[i].id
        datasList.push(dataObject)
      }
    ...
    sureAddShow6() {
      const datasList = []
      for (var i = 0; i < this.deleteList.length; i++) {
        const dataObject = { id: '', collectArea: this.formInline1.collectArea }
        dataObject.id = this.deleteList[i].id
        datasList.push(dataObject)
      }
    ...
    sureAddShow5() {
      const datasList = []
      for (var i = 0; i < this.deleteList.length; i++) {
        const dataObject = { id: '', collectStatus: this.formInline1.collectStatus }
        dataObject.id = this.deleteList[i].id
        datasList.push(dataObject)
      }
    ...
    sureAddShow4() {
      const datasList = []
      for (var i = 0; i < this.deleteList.length; i++) {
        const dataObject = { id: '', important: this.formInline1.importLeave }
        dataObject.id = this.deleteList[i].id
        datasList.push(dataObject)
      }
    ...
    sureAddShow(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const datasList = []
          for (var i = 0; i < this.deleteList.length; i++) {
            const dataObject = { id: '', comment: this.value12 }
            dataObject.id = this.deleteList[i].id
            datasList.push(dataObject)
          }
